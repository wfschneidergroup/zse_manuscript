...
%DIF ADD ./manuscript.tex                                Mon Dec 26 18:55:57 2022
...
\DIFaddendFL \caption{Cutout of the Chabazite framework showing a path (5-6-7-8-9) highlighted with purple bonds, a cycle (3-4-18-19-20-17) highlighted with blue bonds, an 8-MR filled in with pink, and a 4-MR filled in with green. Yellow atoms are Si (T-sites), and red atoms are oxygen. \label{fig:3.1}}
\end{figure}
\end{figure}
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.5\textwidth]{figures/chapter-3/ton-6-10.pdf}
...
\cite{goetzke-properties-1991} }\hskip0pt%DIFAUXCMD
in a Python package called the Zeolite Simulation Environment (ZSE)}\DIFdelend . In ZSE we use the framework put in place by the Atomic Simulation Environment (ASE) \cite{hjorth-larsen-atomic-2017} to handle routine analysis zeolite crystal structures. All graph theory functions are performed used the NetworkX Python package \cite{hagberg-exploring-2008}. 
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.5\textwidth]{figures/chapter-3/goetzke.pdf}
...
\cref{fig:stereo} }\hskip0pt%DIFAUXCMD
that are not able to be captured by the vertex symbol leads us to believe that the vertex symbol is not a complete descriptor, and there is room to define a new descriptor that takes into consideration ring orientation. This has led us to create a new method for listing the rings in the vertex symbol that considers the structural connection of the rings described in \mbox{%DIFAUXCMD
\cref{section:ov}}\hskip0pt%DIFAUXCMD
. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{With this new descriptor }\DIFdelend \DIFaddbegin \DIFadd{, as highlighted in the lower panel of \mbox{%DIFAUXCMD
\cref{fig:stereo}}\hskip0pt%DIFAUXCMD
. To remove this ambiguity, an alternative approach is to \red{need to explain the basis of the modified procedure in words here. 2.5 describes the algorithm but not its physical basis.} Within this algorithm }\DIFaddend MOR T3 and EON T9 would be labeled as: 8\DIFdelbegin \DIFdel{\textsubscript{2}\(\cdot\)}\DIFdelend \DIFaddbegin \DIFadd{\(_{\text{2}} \cdot\)}\DIFaddend 8\DIFdelbegin \DIFdel{\textsubscript{2}\(\cdot\)}\DIFdelend \DIFaddbegin \DIFadd{\(_{\text{2}} \cdot\)}\DIFaddend 5\DIFdelbegin \DIFdel{\textsubscript{2}\(\cdot\)}\DIFdelend \DIFaddbegin \DIFadd{\(_{\text{2}} \cdot\)}\DIFaddend 5\(\cdot\)4\(\cdot\)5, and MON T1 as: 8\DIFdelbegin \DIFdel{\textsubscript{2}\(\cdot\)}\DIFdelend \DIFaddbegin \DIFadd{\(_{\text{2}} \cdot\)}\DIFaddend 8\DIFdelbegin \DIFdel{\textsubscript{2}\(\cdot\)}\DIFdelend \DIFaddbegin \DIFadd{\(_{\text{2}} \cdot\)}\DIFaddend 4\(\cdot\)5\(\cdot\)5\DIFdelbegin \DIFdel{\textsubscript{2}\(\cdot\)}\DIFdelend \DIFaddbegin \DIFadd{\(_{\text{2}} \cdot\)}\DIFaddend 5. The difference is subtle but highlights the distinct structural difference between the two types of T-sites that is not otherwise captured by a vertex symbol.  \DIFdelbegin \DIFdel{Stereochemistry of the rings associated with a T-site could influence the chemical properties we care about such as deprotonation energy, T-site substitution energy, or catalytic properties of reactions happening at that T-site.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We have used this new ordered vertex symbol to characterize }\DIFdelend \DIFaddbegin \DIFadd{We computed the ordered vertex symbols for  }\DIFaddend all 1460 T-sites in the IZA Database.  \DIFdelbegin \DIFdel{We found that using the standard vertex symbol to characterize T-sites there are }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend 649 unique vertex symbols \DIFdelbegin \DIFdel{present. In contrast, there are }\DIFdelend \DIFaddbegin \DIFadd{in the database increases to }\DIFaddend 666 unique ordered vertex symbols\DIFdelbegin \DIFdel{across every T-site. This would imply that not a large amount of vertex symbols contain stereochemical differences.  In \mbox{%DIFAUXCMD
...
\cref{tab:ov}  }\hskip0pt%DIFAUXCMD
lists }\DIFaddend some common T-site vertex symbols \DIFdelbegin \DIFdel{, and their representative }\DIFdelend \DIFaddbegin \DIFadd{and associated }\DIFaddend ordered vertex symbols. \DIFaddbegin \DIFadd{\red{Complete results are presented in the Supplementary?} 
}\DIFaddend 
