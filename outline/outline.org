...
\newpage
* Questions to settle with Bill:
1. Where should vertex symbol go?
2. Discuss the table and make sure we are in agreement on everything
3. Naming the ring conventions:
...
4. There are other ring definitions that I have not included in this study: strong ring, and very strong ring. Should we at least introduce those in the intro? 
...
2. We provide a Python package that has the ability to identify and classify these rings based on previously published and new conventions. Node-based search procedure taking advantage of algorithms developed for general nets.
3. We enumerate all rings, defined as paths with no short-cut, up to 18 T-sites across all known Fws
...
4. Various conventions for associating some subsets of closed-path with no shortcut rings with either a framework or with the T-sites of a framework. We compute and compare conventions across all known frameworks.
5. Some features treated by convention as rings are not rings in a topological sense but are rather defined by the void space they create. 
