...
1. Zeolite framework creates rings, defined as closed paths with no shortcuts. 
2. We provide a Python package that has the ability to find these rings based on previously published and new conventions
3. We enumerate all rings up to 18 T-sites across all known Fws using each of these conventions
...
4. Various conventions for associating some subsets of these rings with either a framework or with the T-sites of a framework. Compute and compare conventions across all known frameworks.
